...
void EFFrame::takeData()
{
	...
	delta = data->get_state_minus_stateFEJ().head<8>();
	delta_prior =  (data->get_state() - data->getPriorZero()).head<8>();



//	Vec10 state_FEJ =  data->get_state_FEJ();
//	state_FEJ.segment<3>(0) = SCALE_XI_TRANS * state_FEJ.segment<3>(0);
//	state_FEJ.segment<3>(3) = SCALE_XI_ROT * state_FEJ.segment<3>(3);
//	state_FEJ[6] = SCALE_A * state_FEJ[6];
//	state_FEJ[7] = SCALE_B * state_FEJ[7];
//	state_FEJ[8] = SCALE_A * state_FEJ[8];
//	state_FEJ[9] = SCALE_B * state_FEJ[9];
//
//	std::cout << "state_FEJ: " << state_FEJ.transpose() << "\n";
